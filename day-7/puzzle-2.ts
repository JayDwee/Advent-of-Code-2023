...
class Hand {
    ...
    setPower() : void {
        ...
        let cardGroupObj : {[index: string]: number} = [...this.cards].reduce((acc, curr) => {
            if (curr == 'J') jokerCount++ 
            else acc[curr] ? acc[curr]++ : acc[curr] = 1 
            return acc
        ...
        if (jokerCount == 5) {
            ...
            cardGroup = Object.values(cardGroupObj).sort()
            cardGroup[cardGroup.length-1] = cardGroup[cardGroup.length-1]+jokerCount
        }
